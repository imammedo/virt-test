...
def run_cpuid(test, params, env):
    ...
    class custom_level(MiniSubtest):
        ...
        def test(self):
            ...
            if params.get("level") is None:
                raise error.TestFail("'level' must be specified in config"
                                     " for this test")
            try:
    ...
    class custom_family(MiniSubtest):
        ...
        def test(self):
            ...
            if params.get("family") is None:
                raise error.TestFail("'family' must be specified in config"
                                     " for this test")
            try:
    ...
    class custom_model(MiniSubtest):
        ...
        def test(self):
            ...
            if params.get("model") is None:
                raise error.TestFail("'model' must be specified in config"
                                     " for this test")
            try:
    ...
    class custom_stepping(MiniSubtest):
        ...
        def test(self):
            ...
            if params.get("stepping") is None:
                raise error.TestFail("'stepping' must be specified in config"
                                     " for this test")
            try:
    ...
    class custom_xlevel(MiniSubtest):
        ...
        def test(self):
            ...
            if params.get("xlevel") is None:
                raise error.TestFail("'xlevel' must be specified in config"
                                     " for this test")
    ...
    def cpuid_to_model_id(cpuid_dump):
        ...
        # 80000004h)
        id = ""
        for i in ('0x80000002', '0x80000003', '0x80000004'):
            r = cpuid_regs_to_dic('%s 0x00' % i, cpuid_dump)
            for v in ('eax', 'ebx', 'ecx', 'edx'):
    ...
    class custom_model_id(MiniSubtest):
        ...
        def test(self):
            ...
            if params.get("model_id") is None:
                raise error.TestFail("'model_id' must be specified in config"
                                     " for this test")
